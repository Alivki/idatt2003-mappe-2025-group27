...
/**
 * A factory class responsible for creating {@link LadderGame} instances with predefined
...
public class BoardGameFactory {
  ...
  /**
   * Creates a {@link LadderGame} instance based on the specified {@link LadderGameType}. The game is
   ...
   * @return A new {@link LadderGame} instance configured with the specified game type.
   * @throws IllegalArgumentException if the {@code LadderGameType} is unknown.
   */
  public LadderGame createLadderGame(LadderGameType ladderGameType) {
    ...
    return new LadderGame(board, config.getNumberOfDice(), config.getNumberOfDieSides());
  }

  /**
   * Creates a {@link LadderGame} instace based on a configuration loaded from a JSON file. The game
   ...
   * @return A new {@link LadderGame} instance configured with the specified JSON file.
   * @throws IOException if an error occurs while reading the JSON file.
   */
  public LadderGame createLadderGameFromJson(String jsonPath) throws IOException {
    ...
    return new LadderGame(config.getBoard(), config.getNumberOfDice(), config.getNumberOfDieSides());
  }
}